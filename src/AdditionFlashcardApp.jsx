...
import { resolveInterestTheme } from './lib/interestThemes';
...
export default function AdditionFlashcardApp() {
  ...
  const handleGenerateSprites = async () => {
    ...
    const jobId = await createSpriteJob(interests, aiRuntime.spriteModel);
    if (jobId) {
  ...
  const ensureAiPlan = useCallback(async (force = false) => {
    ...
    if (runtime.aiEnabled && !aiPlanStatusRef.current?.rateLimited) {
      try {
        const remoteResult = await requestPlan({ personalization: ai });
        if (remoteResult?.rateLimited) {
        ...
        } else if (remoteResult?.plan?.items?.length) {
          const { plan, meta } = remoteResult;
          const usedModel = meta?.used_model || plan.source || 'gemini-2.5-pro';
          const normalized = plan.items.map((item, index) => ({
            id: item.itemId || `${plan.planId || 'gemini'}-${Date.now()}-${index}`,
            a: item.a ?? item.operands?.[0] ?? 0,
            b: item.b ?? item.operands?.[1] ?? 0,
            answer: item.answer ?? (item.a + item.b),
            display: item.display || `${item.a} + ${item.b}`,
            predictedSuccess: item.predictedSuccess ?? ai.targetSuccess,
            difficulty: item.difficulty ?? ai.targetSuccess,
            hints: item.hints ?? [],
            praise: item.praise || '',
            microStory: plan.microStory || '',
            source: usedModel,
            planId: plan.planId || `gemini-${Date.now()}`,
          }));
          ...
          return { reused: false, appended: normalized, plan };
        }
      } catch (error) {
        console.warn('AI planning failed, falling back to local planner.', error);
        setAiPlanStatus((prev) => ({ ...prev, error: error.message || 'AI planning failed.' }));
      }
    }
  ...
  const activeInterestTheme = useMemo(
    () => resolveInterestTheme({
      ...
      themes: learnerProfile.interestThemes || [],
    }),
    [primaryInterest, learnerProfile.interestThemes],
  );
